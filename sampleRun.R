...
normalized_data_GS <- data.frame(normalized_data)
ensembl <- nth(tstrsplit(rownames(normalized_data_GS), split = "\\."), n = 1)
ids <- data.frame(SYMBOL = mapIds(org.Hs.eg.db, keys = ensembl, column = "SYMBOL", keytype = "ENSEMBL", multiVals = "first"))
ids$ensembl <- rownames(ids)
normalized_data_GS$SYMBOL <- ids$SYMBOL
normalized_data_GS <- normalized_data_GS[match(na.omit(unique(normalized_data_GS$SYMBOL)), normalized_data_GS$SYMBOL), ]
rownames(normalized_data_GS) <- normalized_data_GS$SYMBOL
normalized_data_GS <- normalized_data_GS[, !colnames(normalized_data_GS) %in% "SYMBOL"]
colnames(normalized_data_GS) <- colnames(normalized_data)
...
samples$Train[samples$Sample.Name %in% Consistent_Classifications$`Expression ID`] <- "yes"
...
load("/Users/akdes/Library/CloudStorage/Box-Box/baylor/scell_meningioma_ucsf_bcm_sean/scram/091622_SCELL_UCSF_BCM_seuratObj_ann.rda")
new.cluster.ids <- c(
    "other", # 0
    "tumor", # 1
    "microglia", # 2
...
save(list=c("bulk_count_data", "bulk_normalized_data", "samples", "seurat_obj"), file="Meningioma_Data.Rdata")
...
load("/Users/akdes/Library/CloudStorage/Box-Box/baylor//arya_akash_rnaseq_classifier/Meningioma_Data.Rdata")
...
openxlsx::write.xlsx(results, file = "/Users/akdes/CLIPPR_results.xlsx")
...
openxlsx::write.xlsx(results[match(soi,rownames(results)), ], file = "/Users/akdes/CLIPPR_results_updated.xlsx")
...
p<- pheatmap(t(data2[ ,rownames(ann)]), cluster_cols = F, cluster_rows = F, gaps_col = idx, 
    ...
    show_rownames = T, filename = "heatmap_Inhouse.pdf",fontsize_row = 6)


pdf("heatmap_Inhouse.pdf", width=10, height=50)
p
dev.off()
